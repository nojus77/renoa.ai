...
export async function POST(
  ...
  try {
    ...
    // Normalize the service interest to lowercase for matching
    const serviceToMatch = lead.serviceInterest.toLowerCase().trim();

    console.log('Looking for providers with service:', serviceToMatch);
    ...
    // Filter providers that match the service type
    const providers = allProviders.filter(provider => {
      // Check if any of the provider's service types match
      const hasMatchingService = provider.serviceTypes.some(
        serviceType => serviceType.toLowerCase().trim() === serviceToMatch
      );
      return hasMatchingService;
    });

    console.log('Providers matching service type:', providers.length);

    if (providers.length === 0) {
      return NextResponse.json({
        matches: [],
        message: 'No providers found matching this service type',
        recommendation: `Unfortunately, we don't currently have any ${lead.serviceInterest} providers in our network. Please add providers for this service type or contact the lead directly.`,
      });
    }
    ...
    const scoredProviders = providers.map(provider => {
      ...
      if (leadCoords && providerCoords) {
        ...
        // Same city match
        if (lead.city.toLowerCase() === provider.city.toLowerCase()) {
          score += 15;
...
Top ${topMatches.length} Provider Match${topMatches.length > 1 ? 'es' : ''}:
${topMatches.map((m, i) => `
...
Provide a 2-3 sentence recommendation on which provider to assign this lead to and why. Be specific and actionable.`;
    ...
    return NextResponse.json(
      { error: 'Failed to match providers', details: error instanceof Error ? error.message : 'Unknown error' },
      { status: 500 }
    );
  }
}