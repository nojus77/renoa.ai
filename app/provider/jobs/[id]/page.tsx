...
export default function JobDetailPage() {
  ...
  const handlePhotoUpload = async (file: File, type: 'before' | 'during' | 'after') => {
    ...
    try {
      ...
      const response = await fetch(`/api/provider/jobs/${jobId}/photos/upload`, {
        method: 'POST',
        body: formData,
      });
      ...
      // Refresh job to get updated photos
      fetchJobDetails();
    } catch (error: any) {
  ...
  const triggerPhotoUpload = (type: 'before' | 'during' | 'after') => {
    console.log('🖱️ Trigger photo upload:', type);
    ...
    return (
      <div className="min-h-screen bg-zinc-950 flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-zinc-400 text-lg">Loading job details...</p>
        </div>
      </div>
    );
  }
    ...
    return (
      <div className="min-h-screen bg-zinc-950 flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-zinc-300 text-lg mb-2">Job not found</p>
          <button
  ...
  return (
    <div className="min-h-screen bg-zinc-950 pb-8 lg:pb-12">
          ...
          <div className="flex flex-wrap gap-2 lg:gap-3 justify-start">
            ...
            {(job.status === 'scheduled' || job.status === 'in_progress') && (
              <button
                ...
                className="flex items-center gap-2 px-4 py-2 lg:px-5 lg:py-2.5 bg-pink-600 hover:bg-pink-500 rounded-lg transition-all hover:scale-105 active:scale-95 shadow-md hover:shadow-pink-500/50 disabled:opacity-50"
              >
                <Camera className="h-4 w-4 lg:h-4.5 lg:w-4.5 text-white" />
                <span className="text-xs lg:text-sm font-semibold text-white hidden sm:inline">
              ...
              <div className="grid grid-cols-3 gap-6">
                ...
                <div>
                  ...
                  {beforePhotos.length === 0 ? (
                    <button
                      onClick={() => triggerPhotoUpload('before')}
                      disabled={uploadingPhoto}
                      className="w-full aspect-square border-2 border-dashed border-zinc-700 hover:border-emerald-500 rounded-xl flex flex-col items-center justify-center gap-2 transition-all hover:bg-zinc-800/50 group disabled:opacity-50"
                    >
                      <Camera className="h-8 w-8 text-zinc-600 group-hover:text-emerald-500" />
                      <span className="text-xs text-zinc-600 group-hover:text-emerald-400 font-medium">
                        {uploadingPhoto ? 'Uploading...' : 'Add Photos'}
                      </span>
                    </button>
                  ) : (
                ...
                <div>
                  ...
                  {duringPhotos.length === 0 ? (
                    <button
                      onClick={() => triggerPhotoUpload('during')}
                      disabled={uploadingPhoto}
                      className="w-full aspect-square border-2 border-dashed border-zinc-700 hover:border-yellow-500 rounded-xl flex flex-col items-center justify-center gap-2 transition-all hover:bg-zinc-800/50 group disabled:opacity-50"
                    >
                      <Camera className="h-8 w-8 text-zinc-600 group-hover:text-yellow-500" />
                      <span className="text-xs text-zinc-600 group-hover:text-yellow-400 font-medium">
                        {uploadingPhoto ? 'Uploading...' : 'Add Photos'}
                      </span>
                    </button>
                  ) : (
      ...
      {/* Complete Job Modal - Only render when job data is loaded */}
      {job && (
        <CompleteJobModal
