...
const BUCKET_NAME = 'team-messages'; // Reuse team-messages bucket
...
export function validateCustomerMessageFile(file: File) {
...
export async function uploadCustomerMessageMedia({ file, providerId, customerId }: UploadParams) {
  ...
  const path = `customer-${providerId}/${customerId}/${Date.now()}-${randomUUID()}.${fileExt}`;
